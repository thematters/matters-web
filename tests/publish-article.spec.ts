...
test('login and publish article, with likeid already exists', async ({ page }) => {
  ...
  await page.locator('[placeholder="Enter title \\.\\.\\."]').fill('Hello world');
  // Click [aria-label="Enter summary \.\.\."]
  await page.locator('[aria-label="Enter summary \\.\\.\\."]').click();
  // Fill [aria-label="Enter summary \.\.\."]
  await page.locator('[aria-label="Enter summary \\.\\.\\."]').fill('test');
  // Click .ql-editor
