...
test('homepage has article feed and scrollable', async ({ page }) => {
  ...
  const articles = page.getByTestId(TEST_ID.ARTICLE_DIGEST_FEED)
  ...
  const newArticles = page.getByTestId(TEST_ID.ARTICLE_DIGEST_FEED)
...
test('homepage has article feed and can switch to latest feed', async ({
  ...
  console.log(await trendingTab.innerHTML())
  ...
  const latestArticles = page.getByTestId(TEST_ID.ARTICLE_DIGEST_FEED)
...
test('homepage sidebar has recommended tags and users', async ({ page }) => {
  ...
  const tags = page.getByTestId(TEST_ID.TAG_DIGEST_SIDEBAR)
  await expect(tags.first()).toBeVisible()

  // Expect the sidebar has recommended users
  const users = page.getByTestId(TEST_ID.TAG_DIGEST_SIDEBAR)
