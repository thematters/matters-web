...
export const handZoomOut = ($clapButton: HTMLButtonElement) => {
  $clapButton.classList.remove(clapStyles.clapHandZoomIn)
}
export const handZoomIn = _debounce(($clapButton: HTMLButtonElement) => {
  $clapButton.classList.add(clapStyles.clapHandZoomIn)
}, 1000 / 60)
...
export const explode = ($clapButton: HTMLButtonElement) => {
  ...
  // append to button
  $explode.className = `${clapStyles.clapExplode} ${id}`
